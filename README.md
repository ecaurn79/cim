...
## Choix v48 — demo-ia : 3 bugs de l'install WebLLM corrigés

**1. Saisie impossible après chargement (PC)** : #chatInput naît disabled et n'était JAMAIS réactivé au terme de l'installation (seul ask() le réactivait après une première réponse — donc jamais). Le chemin « modèle prêt » active désormais input+send et prend le focus. **2. Faux « Le téléchargement semble bloqué »** : le chien de garde ne tolérait que 12 s sans progression (entre deux shards de 300 Mo ou pendant la compilation GPU, ça dépasse souvent) → seuil porté à 90 s, alerte unique non bloquante, pourcentage suivi (lastPct), et message honnête : « les morceaux déjà reçus restent en cache — recharger la page REPREND là où ça s'est arrêté, rien n'est à refaire ». **3. Tablette Android (erreur maxComputeWorkgroupStorageSize requested=32768 limit=16384)** : limite matérielle des puces mobiles (Mali/Adreno), pas un bug du site. Deux parades : pré-vol AVANT tout téléchargement (requestAdapter → limits, si < 32768 avertissement immédiat + lien vers le générateur d'images qui n'a pas cette contrainte), et traduction propre de l'erreur dans le catch (« limite matérielle, pas un bug du site ; chat sur ordinateur, images ici ») — plus de 0,9 Go téléchargés pour rien. Référence : hugodaniel.com/posts/webgpu-shader-limits (les limites diffèrent par matériel, à vérifier avant requestDevice). Hooks de test window.DEMOIA (state/enableChat) ; validation : chemin ready corrigé vérifié dans la page construite, enableChat réactive input+send en DOM, 23/23 pages sans erreur JS.

## Choix v47 — la collection des anciennes créations + les 7 sites du club en vitrine

**jeux-video.html** : en fin de page, bannière « Les archives du Hub — La collection » vers cimjeuvideo.github.io/collection (analysé : remakes Mario, Tetris, Boulder Dash, Breakout, Tank au Phaser + pages de cours), illustrée par l'atelier rétro aux écrans cathodiques sous guirlande ambrée. **medias.html** : nouvelle section « Les sites que nous avons réalisés. » juste après le paragraphe « chaîne de production qui tourne » — 7 cartes (grille responsive, hover-lift CSS pur selon les patterns 2026) analysées en ligne avant écriture : les DEUX encyclopédies Captain Future (Lattes : pulps+hologramme ; futur : laboratoire du cerveau dans la cloche), les DEUX Buck Rogers (portail domaine public : socle art déco ; roman graphique : planche à trois cases), les DEUX LPGDS (site officiel : tablette cunéiforme en écrin ; énigme 2034 : mur de la cryptographe) et Le Gène de Qin (guerrier en terre cuite derrière la vitre à hélices d'ADN) — chaque image raconte la DIFFÉRENCE entre les déclinaisons. Contraintes respectées : 8 images, JPG seul, 1200 px q74. Validation : 213/213 références valides, 23/23 pages sans erreur JS, lien collection ×1 sur jeux-video, 7 « Visiter le site » sur medias.

## Choix v46 — le réseau de jeux : 8 jeux GitHub Pages intégrés à jeux-video.html

Nouvelle section « Huit jeux de plus sur nos GitHub Pages » sur jeux-video.html, exactement entre la bannière des échecs et Hub of Duty, avec une key art cinématographique et un texte d'accroche par jeu — chacun analysé en ligne avant écriture : **Blackout Protocol** (FPS Secteur 7 au crépuscule), **NEXUS // BLACKSITE** (3 vagues + extraction sur site classifié), **IRON VEIL** (simulateur tactique Highlands/néon), **Le Londres de Sherlock Holmes** (carte canon : 106 lieux, 6 affaires), **Arthurian Legends: The Fallen Crown** (graphic novel 50 chapitres + tactique), **Voyage au centre de la Terre** (RPG Verne 1864, lampes de Ruhmkorff), **Voyages Extraordinaires** (les expéditions verniennes unifiées, survie), **Chronicles of the Cosmic Dread** (enquête lovecraftienne, Arkham 1929). Les 8 liens s'ouvrent dans un onglet (target=_blank rel=noopener). Apport de la recherche : key art = 1 sujet + lumière 3 points + contraste fort avec zones à peine lisibles, objet clé seul en lumière claire (ixiegaming, gamineai). Contraintes respectées : 8 images, JPG seul, 1200 px q74. Validation : 205/205 références valides, 23/23 pages sans erreur JS (harnais /tmp reconstruit après purge : puppeteer + librairies Chrome), ordre échecs → 8 jeux → Hub of Duty vérifié en DOM, preuve shots/jeux-video-github-pages.png.

## Choix v45 — galerie 3D corrigée et ciel embelli : ratios exacts, 40/40 couvertures, starfield spectraL

**Corrections demandées** : (1) les couvertures tronquées — cause trouvée : 14 couvertures sur 40 ont un ratio ≠ 2:3 (0,52 à 0,78) ; chaque fenêtre 3D adopte désormais le ratio EXACT de sa couverture (scale.x = ratio/(2/3), idem cadre et halo), la fiche et le mur de repli passent en hauteur:auto — plus aucune déformation ; (2) 9 couvertures manquantes (404) — cause trouvée : bug de mon pipeline v44 qui déplaçait les JPG d'origine au lieu des PNG convertis ; les 9 restaurées depuis romans-covers-src, 40/40 chargent. **Évolutions starfield** (état de l'art 2026 : couleurs spectrales, scintillement multi-fréquences, bande galactique — CK42BB/procedural-stars-threejs ; sprites ronds CanvasTexture — three.js guide/stackoverflow) : 3 couches d'étoiles spectrales (bleues, blanches, ambre) à scintillement désynchronisé, bande galactique de 900 étoiles sur un cercle incliné (distribution gaussienne), 3 voiles de nébuleuse additifs, sprites ronds partout (plus aucun carré), et flaques de lumière ambrée au pied de chaque fenêtre. **Amélioration galerie bonus** : double-clic = zoom solo sur une couverture (double-clic à vide = retour). Debug mémorable : deux pièges d'ASI (ligne suivant `var starLayers=[];` commençant par `[[` = index, et forEach(…)() après restructuration) — preuves shots/galerie3d-v45*.png (40/40 textures, 62 draw calls, zéro erreur). Validation : 23/23 pages sans erreur JS, 197/197 références valides, 40/40 vignettes.
